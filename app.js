...
const FIREBASE_CONFIG = {
 apiKey: "AIzaSyDIXBOY-aJC-7DtjA60kxw9cwlsVJjVJOI",
  authDomain: "miss-understanding-kalendarz.firebaseapp.com",
  databaseURL: "https://miss-understanding-kalendarz-default-rtdb.europe-west1.firebasedatabase.app",
  projectId: "miss-understanding-kalendarz",
  storageBucket: "miss-understanding-kalendarz.firebasestorage.app",
  messagingSenderId: "40668034192",
  appId: "1:40668034192:web:23936b23e484b6fd071415",
  measurementId: "G-WY7QDEE3LT"
};
...
// ══════════════════════════════════════════════════════════════
// MODUŁ: GRAFIK TYGODNIOWY (Stałe Korepetycje)
// ══════════════════════════════════════════════════════════════

// ─── Firebase paths dla grafiku ───
const SCHED_PATHS = {
  slots:    'scheduleSlots',     // bloki dodane przez nauczyciela (dzień + godziny)
  bookings: 'scheduleBookings',  // rezerwacje uczniów
};

const SchedDB = {
  async getSlots() {
    if (!firebaseDB) return localGet('schedSlots_v1', []);
    const snap = await get(ref(firebaseDB, SCHED_PATHS.slots));
    if (!snap.exists()) return [];
    return Object.entries(snap.val()).map(([id, v]) => ({ ...v, id }));
  },
  async addSlot(slot) {
    if (!firebaseDB) { const l = localGet('schedSlots_v1',[]); l.push(slot); localSet('schedSlots_v1',l); return; }
    await set(ref(firebaseDB, `${SCHED_PATHS.slots}/${slot.id}`), slot);
  },
  async deleteSlot(id) {
    if (!firebaseDB) { localSet('schedSlots_v1', localGet('schedSlots_v1',[]).filter(s=>s.id!==id)); return; }
    await remove(ref(firebaseDB, `${SCHED_PATHS.slots}/${id}`));
  },
  async getBookings() {
    if (!firebaseDB) return localGet('schedBookings_v1', []);
    const snap = await get(ref(firebaseDB, SCHED_PATHS.bookings));
    if (!snap.exists()) return [];
    return Object.entries(snap.val()).map(([id, v]) => ({ ...v, id }));
  },
  async addBooking(booking) {
    if (!firebaseDB) { const l = localGet('schedBookings_v1',[]); l.push(booking); localSet('schedBookings_v1',l); return; }
...
// ─── State ───
let schedSlots    = [];   // bloki nauczyciela (dzień tygodnia + godzina)
let schedBookings = [];   // stałe rezerwacje uczniów
let pendingSchedSlotId = null; // id slotu podczas rezerwacji
...
// ─── RENDER WEEK GRID (widok dla uczniów + admin) ───
function renderWeekGrid() {
  ...
  for (let d = 1; d <= 5; d++) {
    ...
    const daySlots = schedSlots.filter(s => s.dayOfWeek === d)
    ...
    container.innerHTML = daySlots.map(slot => {
      // Znajdź rezerwacje dla tego slotu
      const booked = schedBookings.filter(b => b.slotId === slot.id);
      // Wolne segmenty (1h lub 1.5h) w obrębie slotu
      const freeSegs = getSchedFreeSegments(slot.timeFrom, slot.timeTo, booked);
      const isFull   = freeSegs.length === 0;

      if (isAdmin) {
        // Admin: pokaż blok z info
        const bookedHtml = booked.map(b => `
          <div class="week-slot-badge admin-booked"
               onclick="showSchedBookingDetail('${b.id}')">
            <span class="slot-dot"></span>
            ${b.bookedTimeFrom}–${b.bookedTimeTo} ✓ ${escHtml(b.clientName||'')}
          </div>`).join('');
        const freeHtml = freeSegs.map(seg => `
          <div class="week-slot-badge available"
               onclick="showAdminSchedSlot('${slot.id}')">
            <span class="slot-dot"></span>${seg.from}–${seg.to}
          </div>`).join('');
        return bookedHtml + freeHtml;
      } else {
        if (isFull) {
          return `<button class="week-slot-badge booked" disabled aria-label="Zajęte ${slot.timeFrom}–${slot.timeTo}">
            <span class="slot-dot"></span>${slot.timeFrom}–${slot.timeTo}
          </button>`;
        }
        return freeSegs.map(seg => `
          <button class="week-slot-badge available"
                  onclick="openSchedBooking('${slot.id}','${seg.from}','${seg.to}')"
                  aria-label="Zarezerwuj ${DAY_NAMES[d]} ${seg.from}–${seg.to}">
            <span class="slot-dot"></span>${seg.from}–${seg.to}
          </button>`).join('');
      }
    }).join('');
...
// ─── Wolne segmenty w bloku grafiku (co 1h lub 1.5h) ───
function getSchedFreeSegments(blockFrom, blockTo, booked) {
  const start = timeToMin(blockFrom);
  const end   = timeToMin(blockTo);
  const DURATIONS = [60, 90]; // możliwe długości lekcji
  const taken = booked.map(b => ({
    from: timeToMin(b.bookedTimeFrom), to: timeToMin(b.bookedTimeTo)
  }));

  const segs = new Map(); // key = "from-to"

  DURATIONS.forEach(dur => {
    for (let t = start; t + dur <= end; t += 60) { // co 1h krok
      const tEnd = t + dur;
      const overlap = taken.some(r => t < r.to && tEnd > r.from);
      if (!overlap) {
        const key = `${t}-${tEnd}`;
        if (!segs.has(key)) segs.set(key, { from: minToTime(t), to: minToTime(tEnd) });
      }
    }
  });

  return Array.from(segs.values()).sort((a, b) => a.from.localeCompare(b.from));
}

// ─── Otwieranie modalu rezerwacji stałego terminu ───
window.openSchedBooking = function(slotId, segFrom, segTo) {
  if (!RateLimit.canBook()) { showToast('Limit rezerwacji na dziś. Spróbuj jutro.', 'error'); return; }
  const slot = schedSlots.find(s => s.id === slotId);
  if (!slot) return;
  pendingSchedSlotId = { slotId, segFrom, segTo, slot };

  const dayName = DAY_NAMES[slot.dayOfWeek];
  document.getElementById('schedBookingSlotInfo').innerHTML =
    `📆 <strong>${dayName}</strong> · ⏰ ${segFrom}–${segTo} · stały, tygodniowy termin`;

  // Reset form
  ['schedName','schedEmail','schedPhone','schedMessage','schedExamOther'].forEach(id => {
    const el = document.getElementById(id); if (el) el.value = '';
  });
  ['schedSchoolStage','schedExam','schedLevel'].forEach(id => {
    const el = document.getElementById(id); if (el) el.value = '';
  });
  document.querySelector('input[name="schedDuration"][value="60"]').checked = true;

  const today = toDateStr(new Date());
  const schedStart = document.getElementById('schedStartDate');
  const schedEnd   = document.getElementById('schedEndDate');
  if (schedStart) { schedStart.min = today; schedStart.value = today; }
  if (schedEnd)   { schedEnd.min = today; schedEnd.value = ''; }
  ...
  document.getElementById('examOtherGroup').style.display = 'none';
  const hp = document.getElementById('hpSched'); if (hp) hp.value = '';

  openModal('scheduleBookingModal');
};

// ─── Egzamin inne — pokaż/ukryj textarea ───
...
// ─── Obsługa rezerwacji stałego terminu ───
...
async function handleSchedBooking() {
  ...
  if (!pendingSchedSlotId) { errEl.textContent = 'Błąd — wybierz termin ponownie.'; return; }

  const { slotId, segFrom, segTo } = pendingSchedSlotId;

  const durVal   = document.querySelector('input[name="schedDuration"]:checked')?.value;
  const startDate = sanitize(document.getElementById('schedStartDate').value, 20);
  const endDate   = sanitize(document.getElementById('schedEndDate').value, 20);
  ...
  const exam      = sanitize(document.getElementById('schedExam').value, 50);
  const examOther = sanitize(document.getElementById('schedExamOther').value, 300);
  const level     = sanitize(document.getElementById('schedLevel').value, 10);
  const message   = sanitize(document.getElementById('schedMessage').value, 500);

  // Walidacja
  if (!startDate) { errEl.textContent = 'Podaj datę rozpoczęcia.'; return; }
  if (!endDate)   { errEl.textContent = 'Podaj datę zakończenia.'; return; }
  if (endDate < startDate) { errEl.textContent = 'Data zakończenia musi być po dacie rozpoczęcia.'; return; }
  ...
  if (!exam)               { errEl.textContent = 'Wybierz cel nauki.'; return; }
  if (!level)              { errEl.textContent = 'Wybierz poziom angielskiego.'; return; }

  // Sprawdź czy wybrany czas (segFrom + durVal min) mieści się w segmencie
  const chosenEnd = minToTime(timeToMin(segFrom) + parseInt(durVal));
  if (timeToMin(chosenEnd) > timeToMin(segTo)) {
    errEl.textContent = `Ten czas jest za krótki na ${durVal === '90' ? '1,5h' : '1h'} lekcję. Wybierz inny termin.`;
    return;
  }

  // Sprawdź konflikt
  const conflict = schedBookings.some(b =>
    b.slotId === slotId &&
    segFrom < b.bookedTimeTo && chosenEnd > b.bookedTimeFrom
  );
  if (conflict) { errEl.textContent = 'Ten czas jest już zajęty. Wybierz inny termin.'; return; }

  btn.disabled = true; lbl.textContent = 'Wysyłanie…';

  try {
    const slot = schedSlots.find(s => s.id === slotId);
    const dayName = DAY_NAMES[slot?.dayOfWeek || 1];

    await sendSchedBookingEmail({
      clientName: name, clientEmail: email, clientPhone: phone,
      dayName, timeFrom: segFrom, timeTo: chosenEnd,
      startDate, endDate, stage, exam: exam === 'inne' ? examOther : exam,
      level, message,
    });
    ...
    const booking = {
      ...
      bookedTimeFrom: segFrom, bookedTimeTo: chosenEnd,
      startDate, endDate,
      clientName: name, clientEmail: email, clientPhone: phone,
      stage, exam: exam === 'inne' ? `Inne: ${examOther}` : exam,
      level, message,
      bookedAt: new Date().toISOString(),
    };
    ...
    document.getElementById('schedSuccessMessage').innerHTML =
      `Potwierdzenie wysłano na <strong>${escHtml(email)}</strong>.<br>
       Twój termin: <strong>${dayName}, ${segFrom}–${chosenEnd}</strong><br>
       Od ${startDate.split('-').reverse().join('.')} do ${endDate.split('-').reverse().join('.')} 🎉`;
    openModal('schedSuccessModal');
  } catch (e) {
    console.error(e);
    errEl.textContent = 'Błąd wysyłki. Spróbuj ponownie lub napisz bezpośrednio.';
  } finally {
...
// ─── Email stały termin ───
async function sendSchedBookingEmail({ clientName, clientEmail, clientPhone, dayName,
  timeFrom, timeTo, startDate, endDate, stage, exam, level, message }) {
  const stageLabels = {
    'SP-4':'Klasa 4 SP','SP-5':'Klasa 5 SP','SP-6':'Klasa 6 SP',
    'SP-7':'Klasa 7 SP','SP-8':'Klasa 8 SP','LO-1':'Klasa 1 LO/T',
    'LO-2':'Klasa 2 LO/T','LO-3':'Klasa 3 LO/T','LO-4':'Klasa 4 T',
    'student':'Student/ka','adult':'Dorosły'
  };
  const p = {
    to_name:     clientName,
    ...
    end_date:    endDate,
    phone:       clientPhone,
    school_stage: stageLabels[stage] || stage,
    exam_goal:   exam,
    english_level: level,
  ...
  if (typeof emailjs !== 'undefined') {
    // Używamy tego samego template co booking, możesz stworzyć osobny: template_schedule
...
// ─── ADMIN: Dodawanie slotu do grafiku ───
...
// ─── ADMIN: Lista slotów grafiku ───
function renderAdminScheduleSlots() {
  if (!Session.isAdmin()) return;
  const c = document.getElementById('adminScheduleSlotsContent');
  if (!c) return;

  if (!schedSlots.length) {
    c.innerHTML = `<div class="empty-state"><span class="empty-state-icon">📭</span>Brak terminów w grafiku.</div>`;
    return;
  }
  ...
  c.innerHTML = sorted.map(slot => {
    ...
    const segs = getSchedFreeSegments(slot.timeFrom, slot.timeTo, bks);
    return `
      <div class="admin-slot-item">
        <div style="flex:1;min-width:0">
          <div class="admin-slot-date">${DAY_NAMES[slot.dayOfWeek]}</div>
          <div class="admin-slot-time">⏰ ${slot.timeFrom}–${slot.timeTo}</div>
          ${segs.length ? `<div class="admin-free-segs">Wolne: ${segs.map(s=>`<span>${s.from}–${s.to}</span>`).join('')}</div>` : '<div style="font-size:.78rem;color:var(--text-muted)">Blok pełny</div>'}
          ${bks.map(b => `
            <div class="admin-sub-item">
              ...
              · ${escHtml(b.level||'')} · ${escHtml(b.stage||'')}
              <br><small style="color:var(--text-muted)">${b.startDate} – ${b.endDate}</small>
            </div>`).join('')}
        </div>
        <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;justify-content:flex-end">
          <span class="admin-slot-status ${segs.length?'free':'reserved'}">${segs.length?`${segs.length} wolnych`:'Pełny'}</span>
...
window.deleteScheduleSlot = async function(id) {
  if (!Session.isAdmin() || !confirm('Usunąć ten termin z grafiku?')) return;
...
window.showSchedBookingDetail = function(bookingId) {
  ...
  const stageMap = {'SP-4':'Klasa 4 SP','SP-5':'Klasa 5 SP','SP-6':'Klasa 6 SP','SP-7':'Klasa 7 SP','SP-8':'Klasa 8 SP (egzamin ósmoklasisty)','LO-1':'Klasa 1 LO/T','LO-2':'Klasa 2 LO/T','LO-3':'Klasa 3 LO/T','LO-4':'Klasa 4 T','student':'Student/ka','adult':'Dorosły'};
  alert(
    `👤 ${b.clientName}\n📧 ${b.clientEmail}\n📞 ${b.clientPhone||'—'}\n` +
    `📆 ${slot?DAY_NAMES[slot.dayOfWeek]:''}, ${b.bookedTimeFrom}–${b.bookedTimeTo}\n` +
    `📅 Od: ${b.startDate} Do: ${b.endDate}\n` +
    `🎓 ${stageMap[b.stage]||b.stage}\n🎯 ${b.exam||'—'}\n🌐 ${b.level||'—'}\n` +
    `💬 ${b.message||'—'}`
  );
};
...
window.showAdminSchedSlot = function(slotId) {
  ...
  const info = bks.length
    ? bks.map(b => `${b.bookedTimeFrom}–${b.bookedTimeTo}: ${b.clientName} (${b.clientEmail})`).join('\n')
    : 'Brak rezerwacji';
...
// ─── Real-time listener dla grafiku ───
