...
let currentUser  = null; // Firebase Auth — zalogowany admin lub null
...
if (FIREBASE_CONFIGURED) {
  try {
    ...
    console.log('✅ Firebase DB + Auth aktywne');
  } catch (e) {
...
// Centralny listener stanu logowania — jedyne miejsce, które woła showAdminUI/hideAdminUI
// przy Firebase Auth. Callback jest async, więc DOM i hoistowane funkcje są dostępne.
if (firebaseAuth) {
...
const CONFIG = {
  ...
  EMAILJS_TEMPLATE_ID:     'template_schedule',
...
const DB = {
  ...
  async deleteBooking(id) {
    if (!this.ready) {
      localSet(CONFIG.LOCAL_BOOKINGS, localGet(CONFIG.LOCAL_BOOKINGS, []).filter(b => b.id !== id));
      return;
    }
    await remove(ref(firebaseDB, `bookings/${id}`));
  },
  ...
  onDataChange(callback) {
    ...
    const unsub3 = onValue(ref(firebaseDB, 'subscribers'), () => callback());
...
// ══════════════════════════════════════════════════════
const Session = {
  // ── źródło prawdy: Firebase Auth gdy dostępne, fallback sessionStorage ──
  isAdmin() {
    if (firebaseAuth) return !!currentUser;
    // fallback (brak Firebase Auth — np. localhost bez internetu)
  ...
  // Odświeża timestamp aktywności (używany przez session watcher)
  refresh() { sessionStorage.setItem('kalSessTs', Date.now().toString()); },
};
...
function startSessionWatcher() {
  // Inicjuj timestamp aktywności
  sessionStorage.setItem('kalSessTs', Date.now().toString());
  clearInterval(sessionWatcher);
  sessionWatcher = setInterval(async () => {
    const ts  = parseInt(sessionStorage.getItem('kalSessTs') || '0');
    const idle = Date.now() - ts > CONFIG.SESSION_TIMEOUT_MS;
    if (idle) {
      clearInterval(sessionWatcher);
      showToast('Sesja wygasła — wylogowano automatycznie', 'info');
      if (firebaseAuth) {
        await signOut(firebaseAuth); // onAuthStateChanged wywoła hideAdminUI()
      } else {
        Session.set(false);
        hideAdminUI();
      }
...
function updateTimePreview() {
  ...
  const dur = tMin - fMin;
  if (dur > 90) { prev.textContent = '⚠ Maksymalny czas rezerwacji to 1,5h (90 min)'; prev.className = 'time-selection-preview preview-invalid'; return; }
...
document.getElementById('confirmTimeBtn')?.addEventListener('click', () => {
  ...
  if (timeToMin(to) - timeToMin(from) > 90) { errEl.textContent = 'Maksymalny czas rezerwacji to 1,5 godziny (90 min).'; return; }
  ...
  ['bookName', 'bookEmail', 'bookPhone', 'bookMessage', 'bookExamOther'].forEach(id => { const el = document.getElementById(id); if (el) el.value = ''; });
  ['bookSchoolStage', 'bookExam'].forEach(id => { const el = document.getElementById(id); if (el) el.value = ''; });
  const bookExOtherGrp = document.getElementById('bookExamOtherGroup'); if (bookExOtherGrp) bookExOtherGrp.style.display = 'none';
  ...
  document.getElementById('slotDetailContent').innerHTML = `
    ...
    ${bks.length ? bks.map(bk => `
      <div class="admin-sub-booking">
        ...
        📞 ${escHtml(bk.clientPhone || '—')}<br>
        🎓 ${escHtml(STAGE_LABELS[bk.stage] || bk.stage || '—')}<br>
        🎯 ${escHtml(bk.exam || '—')}<br>
        ${bk.message ? `💬 ${escHtml(bk.message)}` : ''}
...
async function updateAdminStats() {
  if (!Session.isAdmin()) return;
  console.log('[AdminStats] Updating stats...');
  // Pobierz świeże dane z Firebase/localStorage
  const [blocks, bookings, subs] = await Promise.all([
    DB.getBlocks(),
    DB.getBookings(),
    DB.getSubscribers()
  ]);
  console.log('[AdminStats] Data loaded:', { blocks: blocks.length, bookings: bookings.length, subs: subs.length });
  const freeCount = blocks.filter(b => {
    const bks = bookings.filter(bk => bk.blockId === b.id);
    return getFreeSegments(b.timeFrom, b.timeTo, bks).length > 0;
  }).length;
  console.log('[AdminStats] Free count:', freeCount);
  const el = id => document.getElementById(id);
  const activeBookings = bookings.filter(bk => blocks.some(b => b.id === bk.blockId));
  if (el('statFree'))        el('statFree').textContent        = freeCount;
  if (el('statBooked'))      el('statBooked').textContent      = activeBookings.length;
        ...
        <div style="flex:1;min-width:0">
         ...
         ${bks.map(bk=>`
            <div class="admin-sub-item">
              ✓ ${bk.bookedTimeFrom}–${bk.bookedTimeTo}
              · 👤 ${escHtml(bk.clientName)}
              · 📞 ${escHtml(bk.clientPhone||'')}
              <span style="color:var(--text-muted);font-size:.76rem">(${escHtml(bk.clientEmail)})</span>
              · 🎓 ${escHtml(STAGE_LABELS[bk.stage]||bk.stage||'')}
              · 🎯 ${escHtml(bk.exam||'')}
              ${bk.message?`<br><small style="color:var(--text-muted)">💬 ${escHtml(bk.message)}</small>`:''}
              <button class="btn-delete-mini" style="margin-left:8px" onclick="confirmDeleteBooking('${bk.id}')" title="Usuń rezerwację">🗑️</button>
            </div>`).join('')}
        </div>
...
document.getElementById('deleteSlotBtn')?.addEventListener('click', async () => {
  ...
  // Dla Firebase - dane zostaną zaktualizowane przez onDataChange listener
  // Dla localStorage - aktualizuj ręcznie
  if (!DB.ready) {
...
// ══════════════════════════════════════════════════════
// ADMIN — USUWANIE POJEDYNCZEJ REZERWACJI
// ══════════════════════════════════════════════════════
window.confirmDeleteBooking = function(id) {
  ...
  (async () => {
    ...
    if (!DB.ready) {
      allBookings = allBookings.filter(bk => bk.id !== id);
      renderCalendar(currentYear, currentMonth);
      renderAdminSlots();
      updateAdminStats();
    }
    showToast('Rezerwacja usunięta', 'info');
  })();
};
...
async function handleBooking() {
  ...
  const phone   = sanitize(document.getElementById('bookPhone').value, 30);
  const stage   = sanitize(document.getElementById('bookSchoolStage').value, 50);
  const examRaw = sanitize(document.getElementById('bookExam').value, 100);
  const examOth = sanitize(document.getElementById('bookExamOther').value, 300);
  ...
  try {
    await sendBookingEmail({ clientName: name, clientEmail: email, clientPhone: phone,
      clientStage: stage, clientExam: exam, clientMessage: message,
...
async function sendBookingEmail({ clientName, clientEmail, clientPhone, clientStage, clientExam, clientMessage, date, timeFrom, timeTo }) {
  const durMin  = timeToMin(timeTo) - timeToMin(timeFrom);
  const h       = Math.floor(durMin / 60), m = durMin % 60;
  const duration = h > 0 && m > 0 ? `${h}h ${m}min` : h > 0 ? `${h} godzina` : `${m} min`;
  const dateObj  = new Date(date + 'T00:00:00');
  const dayName  = PL_DAYS[dateObj.getDay()];
  const p = {
    to_name:        clientName,
    ...
    duration:       duration,
    phone:          clientPhone || '—',
    school_stage:   STAGE_LABELS[clientStage] || clientStage || '—',
    exam_goal:      clientExam || '—',
    message:        clientMessage || 'Brak wiadomości',
...
async function handleLogin() {
  ...
  if (!loginVal || !pass) { errEl.textContent = 'Wypełnij oba pola.'; return; }
  ...
  if (firebaseAuth) {
    // ── Firebase Auth ────────────────────────────────────────────────
    try {
      await signInWithEmailAndPassword(firebaseAuth, CONFIG.TEACHER_EMAIL, pass);
      document.getElementById('loginUser').value = '';
      document.getElementById('loginPass').value = '';
      closeModal('loginModal');
      showToast('Zalogowano ✓', 'success');
      // showAdminUI() + startSessionWatcher() wywoła onAuthStateChanged
    } catch(e) {
      let msg = 'Błędne hasło.';
      if (e.code === 'auth/too-many-requests')
        msg = 'Zbyt wiele prób — Firebase zablokował konto. Spróbuj za chwilę.';
    ...
    // ── Fallback SHA-256 (brak Firebase Auth) ───────────────────────
    const [uh, ph] = await Promise.all([sha256(loginVal), sha256(pass)]);
...
function showAdminUI() {
  console.log('[showAdminUI] Showing admin UI');
  ...
  console.log('[showAdminUI] Rendering admin slots and stats');
  renderAdminSlots();
  renderAdminSubscribers();
  updateAdminStats();
  renderWeekGrid(); // odśwież grafik w trybie admin
  renderAdminScheduleSlots(); // załaduj listę slotów tygodniowych
}
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Inicjalizacja EmailJS — wymagane przed wysyłką maili
  if (typeof emailjs !== 'undefined') {
    try {
      await emailjs.init(CONFIG.EMAILJS_PUBLIC_KEY);
      console.log('✅ EmailJS zainicjalizowany');
    } catch (e) {
      console.error('❌ Błąd inicjalizacji EmailJS:', e);
    }
  }
  ...
  DB.onDataChange(async () => {
    console.log('[onDataChange] Data changed, reloading...');
    [allBlocks, allBookings] = await Promise.all([DB.getBlocks(), DB.getBookings()]);
    renderCalendar(currentYear, currentMonth);
    if (Session.isAdmin()) { 
      console.log('[onDataChange] Updating admin UI');
      renderAdminSlots(); 
      updateAdminStats(); 
    }
  });

  // Sesja — Firebase Auth obsługuje showAdminUI przez onAuthStateChanged.
  // Fallback (brak Firebase Auth): sprawdź sessionStorage jak poprzednio.
  if (!firebaseAuth && Session.isAdmin()) { showAdminUI(); startSessionWatcher(); }
...
const SchedDB = {
  ...
  async deleteBooking(id) {
    if (!firebaseDB) { localSet('schedBookings_v2', localGet('schedBookings_v2',[]).filter(b=>b.id!==id)); return; }
    await remove(ref(firebaseDB, `${SCHED_PATHS.bookings}/${id}`));
  },
  onDataChange(cb) {
...
window.toggleBookExamOther = function(sel) {
  const grp = document.getElementById('bookExamOtherGroup');
  if (grp) grp.style.display = sel.value === 'inne' ? 'block' : 'none';
};
...
async function handleAddScheduleSlot() {
  ...
  const btn  = document.getElementById('addScheduleSlotBtn');
  btn.disabled = true;
  ...
  try {
    ...
    console.error('Błąd zapisu do Firebase (scheduleSlots):', e);
    fb.textContent = 'Błąd zapisu — sprawdź uprawnienia Firebase lub połączenie.';
        ...
        <div style="flex:1;min-width:0">
          ...
          ${bks.map(b=>`
            <div class="admin-sub-item">
              ...
              <button class="btn-delete-mini" style="margin-left:8px" onclick="confirmDeleteSchedBooking('${b.id}')" title="Usuń rezerwację">🗑️</button>
            </div>`).join('')}
        </div>
        <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;justify-content:flex-end">
...
window.confirmDeleteSchedBooking = function(id) {
  if (!Session.isAdmin()) return;
  if (!confirm('Czy na pewno chcesz usunąć tę rezerwację?')) return;
  (async () => {
    await SchedDB.deleteBooking(id);
    if (!firebaseDB) {
